...
When zipping the project for submission, **exclude** the `vendor/` and `node_modules/` folders. Recipients should run `composer install` after extracting.

Include instructions to create `database/database.sqlite` and run `php artisan migrate` (and optionally `db:seed`).

For GitHub submission, include this README so reviewers can run the project locally.
